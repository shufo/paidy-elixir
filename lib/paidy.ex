defmodule Paidy do
  ...
  def make_request_with_key(method, endpoint, key, body, headers, options) when method in [:put, :post] do
  ...
  * options - request options
  ...
  def make_oauth_token_callback_request(body) do
    rb = Paidy.URI.encode_query(body)
    rh = req_headers(Paidy.config_or_env_key)
      |> Map.to_list
  ...
  defp require_paidy_key do
    case Application.get_env(:paidy, :secret_key, System.get_env "PAIDY_SECRET_KEY") || :not_found do
      :not_found ->
